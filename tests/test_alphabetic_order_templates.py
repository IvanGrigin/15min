...
class T(unittest.TestCase):
 def test_names_and_math(self):
  self.assertEqual({n for n,g in eligible_names()},EXPECTED);self.assertTrue(all(len(n) in (4,5) for n,g in eligible_names()));self.assertEqual(GEN['Артём'],'Артёма');self.assertEqual(GEN['Дарья'],'Дарьи')
