...
Из корня репозитория запустите локальный сайт одной командой:

```bash
python3 run.py
```

Затем откройте [http://127.0.0.1:8090](http://127.0.0.1:8090). Проект использует только
стандартную библиотеку Python; отдельная установка пакетов не нужна. Параметры
сервера при необходимости передаются так: `python3 run.py --host 127.0.0.1 --port 8090`.
