...
Если исходная теорема не позволяет восстановить доказательство без догадки,
соответствующий объект остаётся в `templates.json` для source provenance, но
получает `active: false` и содержательное `exclusion_reason`. Нельзя оставлять
такой шаблон доступным selector-у с правдоподобным, но неполным answer key.
