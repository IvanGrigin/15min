...
## Батч «масштаб и единицы площади» (2026-08-07)

| номера | шаблон | модуль | тест |
|---|---|---|---|
| 243, 261, 880, 881, 1034 | `sheet_weight_by_area` | quantities_units_weight_and_scaling | `tests/test_templates_scale_and_units.py` |
| 109, 114, 596 | `square_of_equal_squares` | plane_geometry_rectangles_squares_and_areas | там же |
| 713 | `square_units_conversion` | quantities_units_weight_and_scaling | там же |
| 774 | `square_frame_around_picture` | plane_geometry_rectangles_squares_and_areas | там же |
| 246, 264 | `two_carpets_overlap` | plane_geometry_rectangles_squares_and_areas | там же |

Двенадцать задач корпуса. Общая ловушка темы — квадратные единицы: в метре
сто сантиметров, а в квадратном метре десять тысяч квадратных. Поэтому
у трёх шаблонов есть отдельный тест, который требует, чтобы наивный
ответ — без перевода единиц, с одинарной рамкой, с линейным отношением
вместо квадратного — отличался от верного. Задача, где эти числа совпали
бы, не проверяет ничего.

В задаче про ковры перекрытие в противоположных углах — квадрат со стороной,
равной выступу, а в соседних — прямоугольник «выступ на меньший ковёр».
Поэтому обе площади вместе определяют комнату однозначно, и тест это
проверяет перебором, а не формулой. Отношение сторон ковров разыгрывается —
вдвое, втрое, вчетверо: в обеих задачах корпуса ковёр вдвое больше, и на
одной этой жеребьёвке готовая формула «три меньших ковра минус выступ»
запоминалась бы вместо разбора. Тест требует, чтобы в выдаче встречались
все три отношения, иначе разнообразие остаётся только заявленным.

Данные: в словарь добавлено существительное «квадратный сантиметр» с явными
формами счёта — «2 квадратных сантиметра», а не «2 квадратного сантиметра»:
при прилагательном стандартное правило счёта неверно.

Изменений в Python нет ни на каком уровне.

## Замер разнообразия и четыре самых бедных шаблона (2026-08-07)

По совету AGENTS.md («сколько различных условий даёт шаблон на полусотне
жеребьёвок») библиотека промерена целиком: медиана 50 из 50, то есть
подавляющее большинство шаблонов не повторяется вовсе. Хвост оказался
коротким, и в нём нашлись четыре шаблона, где разнообразие ограничено
не приёмом, а тем, что автор не дочитал источник.

| шаблон | было | стало | что добавлено |
|---|---|---|---|
| `factorial_value` | 5 условий | 10 | верхняя граница до 13!, как просит источник 1138 |
| `count_numbers_by_length` | 6 | 16 | чётность из источника 1313: «сколько чётных четырёхзначных» |
| `digits_all_from_one_set` | 12 | 20 | «хотя бы одна цифра чётная» из источника 1198 — ветка на дополнение |
| `count_numbers_first_last_classes` | 19 | 30 | последний разряд: ноль, пятёрка, делимость на пять |

Во всех четырёх случаях новое взято из самих задач корпуса, а не выдумано:
источник спрашивал больше, чем печатал шаблон. Ветка «хотя бы одна цифра
чётная» решается не прямым счётом, а через дополнение — из всех чисел
вычитаются те, где все цифры нечётные, — и это другой приём, а не другая
формулировка.

Решатели в тестах научены новым веткам: подсчёт чётности перебором
промежутка, дополнение считается двумя частями отдельно, последний разряд
читается из условия словами.

Изменений в Python нет.

## Три календарных приёма, которых в библиотеке не было (2026-08-07)

| номера | шаблон | модуль | тест |
|---|---|---|---|
| 308, 326 | `consecutive_months_total_days` | calendar_and_weekdays | `tests/test_templates_calendar_new_kinds.py` |
| 1270, 1470 | `palindrome_dates_count` | calendar_and_weekdays | там же |
| 772 | `vacation_days_between_dates` | calendar_and_weekdays | там же |

Отбор шёл по признаку «нет даже родственника»: прежние календарные шаблоны
считали дни недели (`month_weekday_clue_date`, `weekday_lesson_last_visit`)
и сдвигали дату вперёд (`date_after_days`). Здесь три другие мысли.

**Сумма длин месяцев.** «В трёх месяцах подряд обычного года ровно 90 дней —
какой месяц мог быть первым?» Ответов почти всегда несколько, и печатаются
все: вариантов мало и они перечислимы, этим задача отличается от «приведите
пример». Цепочка считается по кругу — декабрь-январь-февраль тоже три месяца
подряд. Високосность названа в условии: иначе у задачи два разных ответа
и ни один нельзя признать неверным.

**Свойство записи даты.** Все зеркальные даты XXI века приходятся на февраль,
и это не совпадение: год начинается на «20», запись читается наоборот, значит
месяц — «02». Дальше день определяется годом однозначно. Из-за этого 29.02
попадает в ответ ровно один раз за век, в 2092 году, и только потому, что он
високосный. Промежуток лет разыгрывается: ответ на весь век один и тот же,
и на нём шаблон давал бы одно условие.

**Расстояние между датами.** Обратная к `date_after_days`: известны обе даты,
спрашивается промежуток. Условие называет первый день отдыха и день выхода
на работу, а не «ушёл» и «вышел»: иначе непонятно, входят ли эти дни в отпуск,
и у задачи два разных ответа.

Изменения в Python на уровне движка: три, все общие.

1. Функции `month_run_starts`, `run_total_days`, `palindrome_dates`,
   `is_palindrome_date`, `month_lengths` и кортеж `MONTH_NOMINATIVE`
   в `calendar_puzzles.py` — настоящие длины месяцев и високосные годы
   формулой не выражаются.
2. Типы параметров `month_run` и `palindrome_dates`.
3. `date_shift` теперь кладёт рядом `<имя>_month_word` — название месяца
   в родительном падеже. Сдвиг почти всегда переходит через границу месяца,
   и без этого значения текст мог назвать только число.

Изменений в Python ради конкретной задачи нет. В словарь добавлено
существительное «дата».

## Батч 1 по каталогу приёмов: числа с общей частью (2026-08-07)

| номера | шаблон | модуль | тест |
|---|---|---|---|
| 61, 81, 609, 629, 833, 1180, 1419, 1791, 2085 | `numbers_differing_in_one_digit` | digits_number_notation_and_cryptarithms | `tests/test_templates_number_records.py` |
| 302, 320 | `sum_of_numbers_by_last_digit` | sequences_progressions_and_sums | там же |

Одиннадцать задач свода. Самая большая непокрытая группа — девять
задач-близнецов про три трёхзначных числа с данной суммой, отличающиеся
только первой цифрой. Кажется, что примеров много, но их ровно один:
сумма равна 100·(сумма цифр) + 3·хвост, и остаток по модулю ста задаёт
хвост однозначно. Из 1900 достижимых сумм единственное решение дают 400,
и шаблон берёт только их.

Изменения в Python на уровне движка: три, все общие.

1. Решатель `common_part_numbers.py` и тип параметра `common_part_numbers`.
2. `parameter_variant` теперь вправе объявить собственный `answer_type`:
   ветка, которая спрашивает число вместо списка чисел, обязана и объявить
   его числом. Прежде тип брался только у канона, и валидатор считал
   исправную ветку сломанной.
3. Проверка «ответ соответствует типу» смотрит на тип выпавшей ветки,
   а не канона.

Изменений в Python ради конкретной задачи нет.

## Батчи А, Б, Д, И по каталогу приёмов (2026-08-07)

| номера | шаблон | модуль | тест |
|---|---|---|---|
| 64, 84, 612, 632, 1127, 1769, 2063 | `bug_king_moves_around_damage` | grid_figures_cuts_and_routes | `tests/test_templates_rates_paths_extremes.py` |
| 383 | `mixed_workers_rate_system` | work_productivity_and_joint_actions | там же |
| 157, 1347, 1666, 1960 | `equal_jars_two_gifts` | word_problems_for_equation_setup | там же |
| 1570 | `path_share_forgotten_item` | motion_speed_and_distance | там же |
| 374 | `best_scores_minimum_total` | logic_problems_and_condition_analysis | там же |
| 472, 453 | `ice_cream_shortfall_multiple` | money_purchases_prices_and_calculations | там же |

Шестнадцать задач свода, четыре группы каталога.

**Ход короля в обход аварии.** Условие само подсказывает ключ: авария 1 × 1
удлиняет путь ровно на шаг. Дальше нужно понять, что обход стоит полусуммы
сторон аварии — не площади и не периметра. Формула найдена перебором и
проверена полным поиском в ширину на квадратах от 17 до 25 для всех
прямоугольников до 13 × 13: 108 случаев, расхождений нет. Тест считает
обход на уменьшенном поле — прирост от стороны квадрата не зависит.

**Система на производительности.** Состав второй смеси отличается от первой
только числом быстрых едоков: если менять и тех и других, вычитание
перестаёт убирать неизвестное и задача становится другой. Это требование
проверяется отдельным тестом.

**Крайний случай при фиксированной сумме.** Решатель в тесте нашёл ошибку
шаблона: наибольший итог не равен «сумма минус один», потому что
отброшенная работа не может быть сколь угодно плохой — остальные не могут
быть лучше потолка. Шаблон печатал 41 там, где перебор давал 36.

Изменений в Python нет: все шесть шаблонов выражаются существующими типами.
В словарь добавлены «сок», «сироп», «работа».

## Сводка

- Шаблонов в библиотеке: 167
- Вселенных с падежами, локациями, предметами и ценностями: 43
- Персонажей с падежами: 252 франшизных + 19 обычных имён
- Существительных с полной парадигмой: 441
...
## Батч групп Д, А, Б и З каталога приёмов

Семь шаблонов, шестнадцать задач свода. Группы Д (наименьший результат при
ограничениях), А (системы на производительности) и Б (движение с возвратом)
закрыты целиком; в группе З сделаны две задачи из трёх.

| шаблон | задачи свода | вопросов | приём |
|---|---|---|---|
| `currency_exchange_shared_commission` | 539, 559, 579, 599 | 4 | доля пропорциональна вкладу, делится сумма за вычетом одной комиссии |
| `vitamin_course_pack_cost` | 247, 224, 1060 | 4 | наименьшая стоимость пачками, купить лишнее бывает дешевле |
| `triathlon_stage_speedups` | 636, 616, 650, 664 | 4 | три независимых этапа, третий не назван |
| `cyclist_overshoot_and_return` | 555 | 3 | ответ не зависит от неизвестных длин пути |
| `rider_letter_between_walkers` | 339, 359, 1783, 2077 | 4 | постоянный разрыв, сближение и погоня |
| `two_pieces_on_chessboard` | 1612, 1906 | 5 | размещения с запретом по взаимному положению |
| `snake_table_marked_cells` | 679 | 3 | змеиная нумерация плюс перестановка |

Задача 618 (турнирная таблица четверых) отклонена как `structured_answer`:
за один балл там спрашивается одно число, а за остальные два — заполненная
таблица целиком, то есть двенадцать чисел. Печатать в ключе только первое
значило бы обрезать задачу до трети.

### Изменения в Python

Два новых решателя и два типа параметра — оба по той же причине, по какой
их писали раньше: перебор в языке выражений невыразим.

1. `problemgen/template_studio/pack_purchase.py` — наименьшая стоимость
   покупки пачками (динамика по числу таблеток, с запасом до размера самой
   большой пачки), плюс `greedy_cost` для отбора жеребьёвок. Тип параметра
   `pack_purchase` кладёт рядом `<имя>_tablets` и `<имя>_greedy`.
2. `problemgen/template_studio/snake_table.py` — нумерация змейкой в обе
   стороны и перебор дополнений частичной расстановки. Тип параметра
   `snake_marks` разыгрывается обратно: сначала целая расстановка, потом
   часть её прячется в условие.

### Что нашли тесты

- Решатель `pack_purchase` подтвердил жадный промах источника: на 126
  таблеток три пачки по 50 (339 рублей) дешевле, чем пачка 120 плюс пачка 50
  (352 рубля). Ограничение `cheap != cheap_greedy` требует этого промаха от
  каждой жеребьёвки — без него задача не отличает понявших от посчитавших
  в лоб.
- Проверка согласования числительных отвергла оборот «в 2021 году»: она
  видит число 2021 перед словом «год» и требует именительного, как при
  счёте лет. Спорить с ней ради антуража незачем — годы в триатлоне названы
  словами («в первый год», «на следующий год»), и заодно из схемы ушёл
  лишний параметр.
- Оборот «Ответ дайте в минутах» ломает проверку сайта: она запрещает слово
  «Ответ» в тексте для ученика, и правильно — ученик читает условие, а не
  инструкцию к ключу. Единицы перенесены в сам вопрос: «Сколько минут…»,
  «За сколько минут…», «Сколько километров в час…».
- У велосипедиста тест проверяет не только ответ, но и то, что он не зависит
  от неизвестных длин: три разных разбиения пути на «до магазина» и «лишний
  отрезок» обязаны дать одно и то же время приближения к дому.
- У доски тест сверяет пары вопросов: «не бьют» и «бьют» вместе обязаны
  покрыть все упорядоченные пары клеток, и то же для королей.
