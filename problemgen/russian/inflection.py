...
class RussianNoun:
    ...
    # Вещественные и собирательные слова (молоко, мёд, лапша) считать нельзя:
    # «3 мёда» — дефект текста, а не редкая форма. Флаг задаётся в данных,
    # проверяют его те места, которые подставляют слово в счётный контекст.
    countable: bool = True

    # Внесловарные свойства предмета, а не слова. Нужны задачам, где число
    # берётся из природы существа: у курицы две ноги, у коровы четыре. Без них
    # шаблон писал бы «у курицы 2 ноги» текстом — и подстановка другого
    # животного немедленно сделала бы условие ложным.
    lemma: str = ""              # ключ словаря; совпадает с nom, кроме составных слов
    legs: int | None = None      # число ног у живого существа, None — не существо
    tags: tuple[str, ...] = ()   # категории: animal, food, transport, person, …
