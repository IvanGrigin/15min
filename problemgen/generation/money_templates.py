...
def _change(t,r,s):
 u,cs=_chars(r,1);price=r.randint(20,300);payment=price+r.randint(1,200);name=cs[0].name;text=f"{name} купил товар стоимостью {count_with_word_ru(price,('рубль','рубля','рублей'))} и внёс {count_with_word_ru(payment,('рубль','рубля','рублей'))}. Сколько рублей сдачи нужно выдать?";return _make(t,text,payment-price,{"price":price,"payment":payment,"role_mapping":{"buyer":name}},s,u,cs)
def _split(t,r,s):
 u,cs=_chars(r,2);share=r.randint(20,300);total=share*2;paid=r.randint(0,share-1);answer=share-paid;a,b=cs;text=f"{normalize_sentence(a.name)} и {b.name} делят счёт в {count_with_word_ru(total,('рубль','рубля','рублей'))} поровну. В счёт первой доли уже внесено {count_with_word_ru(paid,('рубль','рубля','рублей'))}. Сколько рублей осталось внести до равной доли?";return _make(t,text,answer,{"total":total,"paid":paid,"share":share,"role_mapping":{"first":a.name,"second":b.name}},s,u,cs)
