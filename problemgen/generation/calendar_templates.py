...
def _tram(t,r,s):
 route=r.choice([60,72,90,120]);old=r.choice([x for x in range(4,20) if route%x==0]);new=r.choice([x for x in range(4,20) if route%x==0 and x!=old]);answer=route//new;text=f"Трамвай проходит полный круговой маршрут за {count_with_word_ru(route,('минуту','минуты','минут'))}. На маршруте равномерно распределены {count_with_word_ru(old,('трамвай','трамвая','трамваев'))}. После изменения их стало {count_with_word_ru(new,('трамвай','трамвая','трамваев'))}. Каков новый интервал движения в минутах?";return _make(t,text,answer,{"route_period":route,"route_period_minutes":route,"old_trams":old,"new_trams":new},s)
def _date_day(t,r,s):
