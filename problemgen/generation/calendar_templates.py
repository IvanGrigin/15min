...
def _weekday(t,r,s):
 start=date(r.randint(2020,2035),r.randint(1,12),r.randint(1,28));delta=r.randint(1,5000);target=start+timedelta(days=delta);text=f"Сегодня {start.strftime('%d.%m.%Y')}. Укажите ISO-код дня недели (понедельник=1, воскресенье=7) через {delta} дней.";return _make(t,text,weekday_code(target),{"start_date":start.isoformat(),"offset_days":delta},s)
...
def _lcm(t,r,s):
 u,cs=_chars(r,3);periods=[r.randint(2,12),r.randint(3,18),r.randint(4,20)];answer=math.lcm(*periods);names=", ".join(c.name for c in cs);text=f"{names} сегодня встретились. Они приходят в библиотеку каждые {periods[0]}, {periods[1]} и {periods[2]} дней соответственно. Через сколько дней они встретятся снова?";return _make(t,text,answer,{"periods":periods,"role_mapping":{"friends":[c.name for c in cs]}},s,u,cs)
def _music(t,r,s):
...
def _breaks(t,r,s):
 u,cs=_chars(r,1);lessons=r.randint(4,8);lesson=45;short=r.randint(5,20);big=r.randint(20,45);total=lessons*lesson+(lessons-2)*short+big;after=r.randint(120,360);start_minutes=r.randint(480,600);leave=start_minutes+total+after;name=cs[0].name;text=f"{name} посещает {lessons} уроков по {lesson} минут. Между ними одна большая перемена {big} минут, остальные одинаковы. От начала первого урока до ухода проходит {total+after} минут, после последнего урока — ещё {after} минут. Сколько длится короткая перемена?";return _make(t,text,short,{"lesson_count":lessons,"lesson_minutes":lesson,"big_break":big,"after_last":after,"total_until_leave":total+after,"role_mapping":{"student":name}},s,u,cs)
