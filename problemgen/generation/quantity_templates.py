...
def _g(t,r):
 ...
 elif s=='bridge':d=r.choice((3,4,5));river=(d-2)*r.randint(10,80);a=river*d//(d-2);fraction={3:'одной трети',4:'одной четверти',5:'одной пятой'}[d];text=f'Через реку шириной {river} м построен мост. Над каждым берегом находится часть моста, равная {fraction} его полной длины. Найдите длину моста в метрах.';v={'river_width':river,'denominator':d}
 else:a=r.choice((10,20,30,40));final=a//2;text=f'Арбуз весил {a} кг и содержал 99% воды. После высыхания арбуз стал содержать 98% воды. Сколько теперь весит арбуз?';v={'initial_mass':a,'final_mass':final};a=final
