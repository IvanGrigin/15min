...
def _olympiad(t,r,s):
 duration=r.randint(2,10)*60; start=r.randint(7,11)*60; offset=r.choice([60,120,180]); a_end=start+duration; b_start=start-offset; b_end=b_start+duration; gap=b_end-start; text=f"В городах А и Б олимпиада начинается в {fmt(start)} по местному времени и длится одинаково. Город А восточнее Б на {offset//60} ч. Окончание в Б на {gap//60} ч позже местного начала в А. Сколько минут длится олимпиада?";return _make(t,text,duration,{"start_local":start,"offset_minutes":offset,"observed_gap":gap},s)
def _direct(t,r,s):
 departure=r.randrange(24*60); origin=r.choice([-360,-240,-120,0,120,240,360]); destination=r.choice([x for x in [-360,-240,-120,0,120,240,360] if x!=origin]); flight=r.randint(80,720); arrival=absolute_to_local(local_to_absolute(departure,origin)+flight,destination);text=f"Самолёт вылетает в {fmt(departure)} по местному времени города А (UTC{origin//60:+d}) и летит {flight} минут в город Б (UTC{destination//60:+d}). Во сколько минут после полуночи по местному времени Б он прилетит?";return _make(t,text,arrival,{"departure_local":departure,"origin_offset":origin,"destination_offset":destination,"flight_minutes":flight},s)
def _turn(t,r,s):
 start=r.randrange(24*60); origin=r.choice([-240,-120,0,120,240]); dest=origin+r.choice([120,240]); west=r.randint(30,180); east=west*r.choice([2,3,4]); arrival=absolute_to_local(local_to_absolute(start,origin)+east+west,dest); turn=absolute_to_local(local_to_absolute(start,origin)+east,origin);text=f"Из города А (UTC{origin//60:+d}) транспорт выехал в {fmt(start)}, ехал на восток {east} минут, затем вернулся {west} минут и прибыл в город Б (UTC{dest//60:+d}) в {fmt(arrival)}. Во сколько минут после полуночи по времени А был разворот?";return _make(t,text,turn,{"start_local":start,"origin_offset":origin,"destination_offset":dest,"east_minutes":east,"west_minutes":west,"arrival_local":arrival},s)
def _round(t,r,s):
 start=r.randrange(24*60); oa=r.choice([-240,-120,0,120]); ob=oa+r.choice([-120,120,240]); f1=r.randint(60,360); wait=r.randint(30,180); f2=r.randint(60,360); answer=absolute_to_local(local_to_absolute(start,oa)+f1+wait+f2,oa);text=f"Поезд выезжает из А (UTC{oa//60:+d}) в {fmt(start)}, едет {f1} минут в Б (UTC{ob//60:+d}), ждёт {wait} минут и возвращается {f2} минут. Во сколько минут после полуночи по времени А он прибудет?";return _make(t,text,answer,{"start_local":start,"offset_a":oa,"offset_b":ob,"first_leg":f1,"wait":wait,"second_leg":f2},s)
def _multileg(t,r,s):
 u,cs=_chars(r);start=r.randrange(24*60);oa=r.choice([-120,0,120]);ob=oa+r.choice([60,120]);oc=ob+r.choice([0,60,120]);a,b,w=r.randint(60,300),r.randint(30,120),r.randint(60,300);answer=absolute_to_local(local_to_absolute(start,oa)+a+b+w,oc);name=cs[0].name;text=f"{name} вылетает из города А (UTC{oa//60:+d}) в {fmt(start)}, летит {a} минут в Б (UTC{ob//60:+d}), ждёт {b} минут и летит {w} минут в В (UTC{oc//60:+d}). Во сколько минут после полуночи по времени В будет прибытие?";return _make(t,text,answer,{"start":start,"offsets":[oa,ob,oc],"legs":[a,w],"wait":b,"role_mapping":{"traveler":name}},s,u,cs)
def _elapsed(t,r,s):
 u,cs=_chars(r);origin=r.choice([-240,-120,0]);dest=origin+r.choice([120,240,360]);start=r.randrange(24*60);duration=r.randint(300,1500);arrival=absolute_to_local(local_to_absolute(start,origin)+duration,dest);name=cs[0].name;text=f"{name} выехал из А (UTC{origin//60:+d}) в {fmt(start)} и прибыл на следующий день в Б (UTC{dest//60:+d}) в {fmt(arrival)}. Сколько минут продолжалось путешествие?";return _make(t,text,duration,{"start_local":start,"origin_offset":origin,"destination_offset":dest,"arrival_local":arrival},s,u,cs)
