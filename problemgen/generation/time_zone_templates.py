...
def _turn(t,r,s):
 start=r.randrange(24*60); origin=r.choice([-240,-120,0,120,240]); dest=origin+r.choice([120,240]); west=r.randint(30,180); east=west*r.choice([2,3,4]); arrival=absolute_to_local(local_to_absolute(start,origin)+east+west,dest)
 text=f"Транспорт выехал из города А (UTC{origin//60:+d}) в {fmt(start)}, ехал на восток {count_with_word_ru(east,('минуту','минуты','минут'))}, затем повернул и ехал обратно {count_with_word_ru(west,('минуту','минуты','минут'))}. В город Б (UTC{dest//60:+d}) он прибыл в {fmt(arrival)} по местному времени. Во сколько минут после полуночи по времени города Б он прибыл?"
 return _make(t,text,arrival,{"start_local":start,"origin_offset":origin,"destination_offset":dest,"east_minutes":east,"west_minutes":west,"arrival_local":arrival},s)
def _round(t,r,s):
 start=r.randrange(24*60); f1=r.randint(60,360); wait=r.randint(30,180); f2=r.randint(60,360); answer=(start+f1+wait+f2)%(24*60);text=f"Поезд выезжает из А в {fmt(start)}, едет до Б {count_with_word_ru(f1,('минуту','минуты','минут'))}, ждёт там {count_with_word_ru(wait,('минуту','минуты','минут'))} и возвращается в А за {count_with_word_ru(f2,('минуту','минуты','минут'))}. Во сколько минут после полуночи он прибудет в А?";return _make(t,text,answer,{"start_local":start,"first_leg":f1,"wait":wait,"second_leg":f2},s)
def _multileg(t,r,s):
...
def _elapsed(t,r,s):
 u,cs=_chars(r);origin=r.choice([-240,-120,0]);dest=origin+r.choice([120,240,360]);start=8*60
 # The fixed morning start and 24–29-hour duration guarantee that the local
 # arrival date in B is exactly the following day for every allowed offset.
 duration=24*60+r.randint(0,300);absolute_arrival=local_to_absolute(start,origin)+duration;arrival_local_unbounded=absolute_arrival+dest
 if arrival_local_unbounded//(24*60)!=1: raise TimeZoneTemplateError("Нарушен контракт даты прибытия")
 arrival=arrival_local_unbounded%(24*60)
 text=f"Поездка из А (UTC{origin//60:+d}) началась в {fmt(start)} и завершилась на следующий день в Б (UTC{dest//60:+d}) в {fmt(arrival)}. Сколько минут продолжалось путешествие?"
 return _make(t,text,duration,{"start_local":start,"origin_offset":origin,"destination_offset":dest,"arrival_local":arrival,"arrival_day_offset":1},s,u,cs)
