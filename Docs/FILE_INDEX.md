...
- реестр занятости тематических веток авторинга;
- фиксирует владельца, статус, прогресс и границы редактирования каждой активной ветки;
- используется до начала параллельной работы и меняется на статус «свободна для перепроверки» после открытия PR.
