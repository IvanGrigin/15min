...
- совместимый статичный каталог неизменяемых текстов математических задач;
...
- используется каталогом, статичным генератором и legacy worksheet-service;
- не является one-to-one копией полного cleaned-корпуса: production selector
  сайта читает проверенные тематические наборы из `data/templates/problem_sets/`.
...
- строит one-to-one overlay только по явной команде `rebuild_catalog()`;
- при обычной проверке текущего статичного каталога использует его канонический
  loader, а не ошибочно требует покрыть весь read-only cleaned-корпус.
