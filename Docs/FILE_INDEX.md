...
- группа C закрыта: 280 строк C01–C12 привязаны к авторским шаблонам, общий счётчик `tasks_done = 433`.
...
- содержит независимый перебор 30 seed для 12 семейств группы C.
...
- содержит авторские модели C01–C12 для задач на цифры, позиционную запись и блоки последовательных чисел;
...
- реализует C-helper'ы подсчёта чисел по цифрам, суммы цифр и соседней чётности, а также стратегии `c02_…` … `c12_…`.
