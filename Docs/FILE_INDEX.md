...
- листы G01–G12 отмечены `done` и ссылаются на авторские runtime-шаблоны `geometry_g01_001`–`geometry_g12_001`.
...
- независимо перебирает целочисленные стороны прямоугольников и позиции квадратов в сетке, сверяя helpers группы G.
...
- для группы G bridge-записи заменены 12 авторскими шаблонами планиметрии и измерений; G01 и G12 используют составной ответ, G05/G08 — runtime helpers.
...
- содержит стратегии `geometry_g01_*`–`geometry_g12_*` и helpers `count_integer_rectangle_sides`/`grid_square_count` для G05 и G08.
