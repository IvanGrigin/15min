...
- листы F01–F11 закрыты авторскими runtime-шаблонами (160 строк корпуса; у F08 нет строк корпуса, но есть шаблон).
...
- содержит независимый перебор 30 seed для каждого из 11 комбинаторных семейств группы F.
...
- содержит авторские шаблоны `comb_…_f01_001` … `comb_…_f11_001`, заменившие bridge-записи F;
...
- принимает плейсхолдеры `text` для текстовых значений, возвращаемых стратегией и не требующих числовых constraints;
...
- реализует стратегии `comb_…` и белые функции `factorial`, `perm_multiset`, `perm_unrank`, `weighted_paths_3x3` для группы F.
