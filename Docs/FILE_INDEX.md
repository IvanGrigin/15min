...
- строки K01--K08 заполнены авторскими runtime-шаблонами и используются при
  контроле прогресса группы K.
...
- воспроизводимо заменяет bridge-записи K01--K08 в
  `data/templates/problem_templates.json` авторскими шаблонами;
- одновременно обновляет поля runtime-шаблона и статус `done` только у строк
  K01--K08 в `data/source_index/per_task_template_worklist.json`;
- используется при авторинге группы K, не затрагивает другие листья.
...
- независимо сверяет ответы восьми авторских модулей группы K на 40 seed.
...
- содержит авторские модули K01--K08 (логика, инварианты и достижимость),
  которые заменили bridge-шаблоны этих листьев;
...
- содержит белые helper-функции и стратегии K01--K08; они отделяют
  инвариант/solver от текстов JSON-шаблонов.
