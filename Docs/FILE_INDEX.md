...
- листы H01–H09 и I01–I08 заполнены authoring-шаблонами и имеют `status = done`;
...
- регрессия авторских шаблонов H01–H09;
- независимо перебирает малые клетчатые сетки, включая внутренние прямоугольные дырки, и сверяет solver-helper'ы перегородок;
- сверяет ответы всех H-шаблонов с независимыми вычислениями на нескольких seed.

Связи:

- тестирует `problemgen/generation/template_generator.py` и H-записи `data/templates/problem_templates.json`.
...
- регрессия авторских шаблонов I01–I08;
- сверяет календарные вычисления, часовые пояса, ход часов и цифровое табло с независимыми расчётами.

Связи:

- тестирует временные helper'ы и I-записи `data/templates/problem_templates.json`.
...
- содержит авторские шаблоны `h01_*`…`h09_*` и `i01_*`…`i08_*`, заменившие bridge-записи групп H и I;
...
- содержит solver-helper'ы перегородок сетки и времени, а также стратегии `h01_*`…`h09_*` и `i01_*`…`i08_*`.
