...
### Авторские шаблоны группы K: логика, инварианты и алгоритмы

Задача:

- заменить все bridge-заглушки листьев K01--K08 на проверяемые авторские
  шаблоны и зафиксировать все 159 строк группы K в ворклисте.

Что сделано:

- удалены 72 bridge-записи `tree_k01`--`tree_k08`; добавлены 8 авторских
  шаблонов, по одному на тематическое семейство K01--K08;
- K02 строит три линейных равенства из заранее выбранного целого `x`; ровно
  одна строка намеренно неверна;
- K03 и K08 проверяют достижимость по НОД, K04 покрытие домино по чётности,
  K05 существование арифметической прогрессии по сумме, а K06 восстанавливает
  состав трёх цветов из минимальных гарантированных выборок;
- K04/K05/K03/K08 возвращают текстовый ответ «можно»/«нельзя» через новые
  белые helper-функции; K01 и K07 используют фиксированную структуру и
  solver-перебор, который подтверждает единственность ответа;
- добавлен узкий идемпотентный скрипт замены каталога и строк ворклиста;
  K01--K08 отмечены `done` (159 задач, всего 193/2121);
- новый тест независимо от `answer_formula` и runtime helper-ов сверяет все
  восемь типов на 40 seed для каждого модуля.
...
- `scripts/replace_group_k_templates.py` — воспроизводимо удаляет только
  bridge-шаблоны K01--K08, добавляет авторские записи и обновляет связанные
  строки ворклиста.

Проверки:

- `python3 -m unittest tests/test_template_generator.py -v` — 18 тестов, OK;
- независимая сверка K01--K08: 40 seed × 8 модулей, включая brute-force для
  двух solver-задач и отдельные расчёты НОД, чётности, суммы и выборок;
- ручной CLI-прогон K01, K02, K03, K04 и K06 подтвердил рендер без
  неразвёрнутых плейсхолдеров и ожидаемые типы ответов.

Заметки для следующего агента:

- для K03/K08 невозможные случаи специально строятся с общим делителем
  шагов больше 1: не подбирать их циклом при НОД = 1;
- K01/K07 намеренно фиксированы, потому что их ценность — в прозрачном
  solver-переборе и единственности ответа, а не в случайной словесной форме.
