...
### Единая команда запуска локального сайта

Дата: 2026-07-21

Что сделано:

- добавлен тонкий launcher `run.py`, который запускает текущий локальный сайт
  одной командой из корня: `python3 run.py`;
- launcher не дублирует конфигурацию HTTP-сервера и передаёт существующие
  аргументы `--host` и `--port` в `scripts/run_site.py`;
- обновлены корневой README, web-инструкция, индекс и навигация.

Изменённые файлы:

- `run.py`;
- `README.md`, `DOCUMENTATION.md`, `Docs/WEB_GENERATION.md`,
  `Docs/FILE_INDEX.md`, `Docs/VECTOR_TREE.md`, `Docs/WORK_LOG.md`.

Новые файлы:

- `run.py` — единая пользовательская точка запуска; использует
  `scripts/run_site.py` и через него `problemgen/web/worksheet_site.py`.

Проверки:

- полный `python3 -m unittest discover -s tests` выполнен с доступом к
  loopback HTTP-сокету.

Дополнительно:

- незакоммиченные материалы по отдельной комбинаторной теме сохранены в git,
  но явно оставлены вне runtime-каталога: у них пока нет source accounting,
  генератора и независимых seed-проверок.
