...
### Авторские шаблоны группы A: арифметика и выражения

Что сделано:

- заменены bridge-заглушки листьев A01–A03 и A05–A08 семью авторскими шаблонами; ранее готовый A04 сохранён;
- в ворклисте отмечены `done` все 181 задачи группы A;
- добавлены стратегии арифметических выражений, точного деления, первой цифры и обратной операции с отбрасыванием последней цифры; helpers `first_digit` и `inverse_truncated_twice` включены в безопасный белый список.
...
- генерация листьев A01–A08 на уровнях 1, 5 и 10; bridge-шаблонов A не осталось.

Заметки для следующего агента:

- шаблон A04 не переписывался: он уже был авторским и продолжает закрывать соответствующие 23 задачи ворклиста.
