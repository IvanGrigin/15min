...
### Группа B, лист B10 — передача денег

Что сделано:

- девять bridge-заглушек B10 заменены авторским шаблоном `b10_equal_money_transfer_001`;
- добавлена стратегия `b10_equal_transfer`, выбирающая чётную итоговую разность; 17 записей B10 отмечены `done`.
...
- независимая сверка 40 генераций: передача меняет разность между равными начальными суммами ровно вдвое;
- `python3 -m unittest discover -s tests` — 42 теста, OK.

Заметки для следующего агента:

- B10 намеренно выбирает только чётную разность, поэтому передаваемая сумма остаётся целой.


## 2026-07-15

### Группа B, лист B09 — прямая пропорциональность

Что сделано:

- девять bridge-заглушек B09 заменены авторским шаблоном `b09_direct_cost_001`;
- добавлена стратегия `b09_direct_proportion` с целой ценой единицы; 26 записей B09 отмечены `done`.
...
- независимое сложение цены единицы 40 раз совпало с ответами генератора;
- `python3 -m unittest discover -s tests` — 42 теста, OK.

Заметки для следующего агента:

- B09 держит единицы совместимыми и строит числа из целой цены за единицу.


## 2026-07-15

### Группа B, лист B08 — проценты и концентрация

Что сделано:

- девять bridge-заглушек B08 заменены авторским шаблоном `b08_water_loss_001`;
- добавлена стратегия `b08_concentration`, выбирающая совместимые проценты и целую массу; 28 записей B08 отмечены `done`.

Измененные файлы:

- `problemgen/generation/template_generator.py` — стратегия концентрации;
- `data/templates/problem_templates.json`;
- `data/source_index/per_task_template_worklist.json`;
- `Docs/WORK_LOG.md`.
...
- независимая сверка 40 генераций по сохранению массы сухого вещества;
- `python3 -m unittest discover -s tests` — 42 теста, OK.

Заметки для следующего агента:

- пары процентов подобраны так, чтобы обе массы были целыми; вода меняется, сухое вещество не меняется.


## 2026-07-15

### Группа B, лист B07 — совместная работа

Что сделано:

- девять bridge-заглушек B07 заменены авторским шаблоном `b07_joint_work_rate_001`;
- использована существующая стратегия `joint_work_two`; 12 записей B07 отмечены `done`.
...
- независимое почасовое моделирование 40 генераций совпало с ответами;
- `python3 -m unittest discover -s tests` — см. коммит листа.

Заметки для следующего агента:

- стратегия гарантирует целые почасовые производительности, поэтому ответ остаётся целым.


## 2026-07-15

### Группа B, лист B06 — системы цен

Что сделано:

- девять bridge-заглушек B06 заменены авторским шаблоном `b06_two_receipts_001`;
- использована существующая стратегия `price_system_two_receipts`; 12 записей B06 отмечены `done` с ответом `[цена A, цена B]`.
...
- независимый перебор цен от 1 до 300 для 40 генераций подтвердил единственную пару цен;
- `python3 -m unittest discover -s tests` — см. коммит листа.

Заметки для следующего агента:

- порядок multi-ответа фиксирован: цена товара A, затем цена товара B.


## 2026-07-15

### Группа B, лист B05 — головы и ноги

Что сделано:

- девять bridge-заглушек B05 заменены авторским шаблоном `b05_heads_and_legs_001`;
- использована существующая стратегия `heads_and_legs`; 28 записей B05 отмечены `done` с ответом `[гномы, пони]`.
...
- независимый полный перебор пар количеств на 40 генерациях дал ровно одно решение, совпадающее с ответом;
- `python3 -m unittest discover -s tests` — см. коммит листа.

Заметки для следующего агента:

- порядок multi-ответа фиксирован: сначала гномы (2 ноги), затем пони (4 ноги).


## 2026-07-15

### Группа B, лист B04 — займы и взаиморасчёты

Что сделано:

- девять bridge-заглушек B04 заменены авторским шаблоном `b04_equal_payment_001`;
- использована существующая стратегия `equal_payment`; 34 записи B04 в ворклисте отмечены `done`.
...
- независимая сверка 40 генераций: итоговый перевод равен займу плюс недостающий до равной доли вклад;
- `python3 -m unittest discover -s tests` — см. коммит листа.

Заметки для следующего агента:

- шаблон отделяет займ от оплаты покупки, поэтому не смешивает два денежных потока.


## 2026-07-15

### Группа B, лист B03 — отношения и части

Что сделано:

- девять bridge-заглушек B03 заменены авторским шаблоном `b03_ratio_parts_total_001`;
- использована существующая стратегия `ratio_sum`; 45 записей B03 в ворклисте отмечены `done` с составным ответом двух частей.
...
- независимый перебор всех возможных пар частей для 40 генераций подтвердил единственную пару и порядок ответа;
- `python3 -m unittest discover -s tests` — см. коммит листа.

Заметки для следующего агента:

- B03 возвращает `multi`: сначала первая, затем вторая часть в указанном отношении.


## 2026-07-15

### Группа B, лист B02 — суммарный возраст

Что сделано:

- девять bridge-заглушек B02 заменены авторским шаблоном `b02_age_total_count_001`;
- использована существующая стратегия `age_friends`; 14 записей B02 в ворклисте заполнены и отмечены `done`.

Измененные файлы:

- `data/templates/problem_templates.json` — авторский шаблон B02 вместо bridge-записей;
- `data/source_index/per_task_template_worklist.json` — разметка B02;
- `Docs/WORK_LOG.md`.
...
- независимая сверка 40 генераций: прирост суммарного возраста равен `число друзей × число лет`;
- `python3 -m unittest discover -s tests` — см. коммит листа.

Заметки для следующего агента:

- для B02 переиспользуется общая стратегия возраста; формула намеренно не зависит от сюжетных персонажей.


## 2026-07-15

### Группа B, лист B01 — линейные уравнения

Что сделано:

- девять bridge-заглушек B01 заменены двумя авторскими шаблонами линейных уравнений: с точным делением и со скобками/умножением;
- в `problemgen/generation/template_generator.py` добавлена стратегия `b_linear_equation_chain`: сначала выбирается целый `x`, затем из него вычисляется правая часть уравнения;
- все 86 записей B01 в ворклисте получили текст шаблона, стратегию, формулу `x`, тип ответа `number` и статус `done`.

Измененные файлы:

- `problemgen/generation/template_generator.py` — стратегия чисел для B01;
- `data/templates/problem_templates.json` — `b01_linear_equation_division_001` и `b01_linear_equation_nested_002` вместо bridge B01;
- `data/source_index/per_task_template_worklist.json` — разметка задач листа B01;
- `Docs/WORK_LOG.md`.
...
- независимый brute-force перебор кандидатов `x` от 1 до 100: по 40 генераций каждого из двух шаблонов, единственное решение совпадает с ответом;
- `python3 -m unittest discover -s tests` — 42 теста, OK (`python` в окружении отсутствует, поэтому использован эквивалентный `python3`).

Заметки для следующего агента:

- B01 намеренно не решает уравнение формулой: целый корень выбирается до построения условия, поэтому деление точное и ответ остаётся целым.
