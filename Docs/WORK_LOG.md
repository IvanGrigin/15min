...
## 2026-07-24 — Миграция русских формулировок active templates

- Добавлен явный data-driven слой форм и рода для approved-персонажей; age,
  money и multiplication-card шаблоны используют только проверенные падежи и
  местоимения.
- Переписаны двусмысленные или неестественные условия возраста, карточек,
  счёта, часов, трамвая, клетчатой буквы, моста и арбуза; read-only источники
  не изменялись.
- `parity_001` возвращён к точному контракту чередующейся чётности: 5 разных
  кандидатов, 2–5 подходящих, независимая проверка списка и суммы.
- Добавлены migration report, focused 100-seed тесты и сквозной lint 28
  selector-модулей × 100 seed. Следующему агенту: не добавлять косвенные формы
  нового имени без записи в `reviewed_character_forms.json`.

## 2026-07-20 — Отчёт об исключённых задачах сайта

- Добавлен воспроизводимый сценарий, который собирает тексты всех source-accounting исключений в `outputs/generated/excluded_site_problems.md` и отдельно перечисляет модули без manifest.

## 2026-07-20 — Аудит формулировок Module 29A

- Возвращено обязательное условие «Настоящий алфавитный порядок букв неизвестен» во все пять runtime-семейств; тесты защищают обе ключевые фразы задачи.

## 2026-07-20 — Модуль 29A: алфавитный порядок

- Добавлен `alphabetic_order`: 13/13 источников в пяти word-answer семействах, точные name→alphabet и `n!`/`n^n`.
- Автоматически отфильтрованы имена с повторяющимися буквами и длиной не 4–5; активны 13 допустимых имён с проверенными родом и родительным падежом.

## 2026-07-19 — Языковая регрессия персонажей и контекст задач

- Убраны мужские прошедшие формы после произвольно выбранных персонажей в модулях подсчёта, часов, денег и работы; формулировки теперь используют нейтральные конструкции.
- Задача об одной ошибке в произведениях получила полный контекст карточек и ролево-нейтральный вопрос вместо «ошибившегося ученика».
- Добавлены регрессии с персонажем Нюшей; аудит 100 seed каждого зарегистрированного модуля не нашёл старых форм.

## 2026-07-19 — Нумерация модулей в selector сайта

- Все пункты ручного выбора модулей получили двухзначный номер в стабильном порядке ответа `/api/templates`; `module_id` и генерация не менялись.
- Обновлены frontend-документация и регрессионная проверка selector.

## 2026-07-19 — Модуль 31: текстовые задачи на составление уравнений

- Учтены 45/45 уникальных источников: 22 активных номера в 10 exact-integer семействах, 23 исключения с точными причинами.
- Добавлены каталог, generator, validator, тесты и selector сайта; именованная семья выбирает approved-персонажей одной вселенной без склонения имён.
- Проверки: focused unit/site, 300 seed-прогонов каждого шаблона, JSON и diff-check перед коммитом.

## 2026-07-19 — Модуль 30: логические задачи и анализ условий

- Учтены 37/37 уникальных источников: 9 активных номеров объединены в 3 exact-integer семейства, 28 исключений имеют точные причины в manifest.
- Добавлены JSON-каталог, generator с независимыми solver-ами, validator и тесты; именованная семья выбирает разных approved-персонажей одной вселенной без склонения имён.
- Сайт получил selector/dispatch модуля 30; заодно зарегистрирован ранее добавленный модуль 28 в `VERIFIED_MODULE_IDS` и статистике покрытия.
- Проверки: focused unit/site, 900 seed-прогонов validator, JSON и diff-check перед коммитом.

## 2026-07-19 — Модуль 22: головы, ноги, колёса и подсчёт объектов

- Source accounting 29/29, все источники активны в трёх exact-integer семействах.
- Добавлены catalog, generator, validator, tests и site integration; сайт сохраняет модули 09–22.

## 2026-07-19 — Модуль 21: возраст и поколения

- Source accounting 15/15, все источники активны в четырёх exact-integer семействах.
- Добавлены catalog, generator, validator, tests и site integration; сайт сохраняет модули 09–21.
...
- Добавлены catalog, generator, validator, tests, site integration, docs и KB; сайт сохраняет модули 09–19.
- Проверки: focused — 3 OK; validator — 800/800; site smoke и diff-check завершены перед коммитом.
...
- Добавлены 7 exact-integer семейств для цифровых табло, часов с разным ходом, башенных дат, расписаний, кукушки и обратных стрелок.
- Именованные стратегии выбирают approved-персонажей одной вселенной и сохраняют имена в именительном падеже.
- Добавлены catalog, generator, validator, tests, site integration, русская документация и KB; stacked-сайт сохраняет модули 09–16.
- Проверки: focused — 26 OK; validator — 1400/1400; расширенная регрессия — 99 OK; полный suite — 201 OK и 8 прежних legacy-сбоев; HTTP smoke и diff-check завершены перед коммитом.
...
Дополнение после проверки сайта: ветка модуля 10 объединена с ещё не влитой
веткой `authoring/module-09-factors`, чтобы текущий checkout показывал оба
последовательных модуля. В catalog, metadata, runtime-dispatch и регрессионных
счётчиках сохранены одновременно 7 шаблонов модуля 09 и 12 шаблонов модуля 10.
...
### База знаний агентов на TriliumNext Trilium (локально + git-зеркало)

Что сделано:

- развёрнута иерархическая база знаний для агентов: локальный TriliumNext
  Trilium без docker и без сторонних серверов, слушает `127.0.0.1:8481`
  (loopback, `noAuthentication`); checkout переиспользуется, каталог данных —
  `../tools/trilium-data-15min` вне репозитория;
- добавлен launcher `scripts/kb_server.mjs` (сам находит собранный checkout,
  создаёт config.ini, инициализирует свежую БД) и CLI `scripts/kb.mjs`
  (ETAPI без зависимостей: `info/ensure/put/get/append/tree/search`,
  `sync-out`/`sync-in`);
- совместная работа — через git: markdown-зеркало `knowledge/` (ветка → каталог
  + `_note.md`, лист → `<название>.md`); у каждого разработчика свой Trilium,
  истина — зеркало в репозитории;
- иерархия наполнена реальным содержимым под этот проект: `00 Обзор` (стартовый
  набор чтения, экономящий токены), `10 Карта проекта` (слои, три маршрута
  авторинга, инварианты), `20 Семейства шаблонов` (8 карточек: production,
  арифметика, последовательности, промежутки, делимость, цифры, recovery,
  олимпиада), `30 Регрессии` (205 хрупких шаблонов, медленная digits-валидация,
  известные legacy-сбои каталога), `40 Эксплуатация`, `90 Журнал` (append-only);
- контракт `Docs/KNOWLEDGE_BASE.md` (иерархия, workflow, параллельная работа,
  запреты) и переносимая инструкция + промт `Docs/AGENT_KB_BOOTSTRAP.md`.

Измененные файлы:

- `AGENTS.md` — использование БЗ сделано обязательным для всех агентов:
  добавлена в источники истины, в «что читать перед началом» (`git pull` +
  `sync-in` + чтение `00 Обзор`), в «что делать после завершения» (обновить
  заметку раздела, разбор регрессии, строка в журнал, `sync-out`, коммит
  зеркала) и в короткую инструкцию;
- `Docs/FILE_INDEX.md` (записи о `scripts/kb*.mjs`, `knowledge/`, новых доках);
- `Docs/WORK_LOG.md`.

Новые файлы:

- `scripts/kb_server.mjs`, `scripts/kb.mjs`;
- `knowledge/` (зеркало дерева заметок);
- `Docs/KNOWLEDGE_BASE.md`, `Docs/AGENT_KB_BOOTSTRAP.md`.

Проверки:

- `node --check` для обоих скриптов — OK;
- сервер поднят вживую: `/api/health-check` → ok, `kb.mjs info` → Trilium 0.103.0;
- `sync-in` наполнил дерево; раунд-трип `sync-out` во временный каталог —
  `diff -r` пустой; `append`/`get`/`search` проверены.

Заметки для следующего агента:

- перед работой: `git pull` → `node scripts/kb.mjs sync-in "15min" knowledge`;
- после задачи: обновить заметку раздела, `append` строку в `90 Журнал`,
  `sync-out` и закоммитить `knowledge/` в ту же ветку, что и код;
- каталог данных Trilium и токены в git не попадают; БЗ не заменяет
  `Docs/WORK_LOG.md` — здесь по-прежнему полная запись, в журнале БЗ строка.


## 2026-07-15

### Единый протокол корректных шаблонов для агентов

Что сделано:

- проверено существующее руководство `Docs/TEMPLATE_AUTHORING_GUIDE.md`: production-цикл, ограничения, стратегии, независимая сверка и DoD уже были описаны;
- руководство расширено актуальным выбором между production-шаблоном, архивным recovery-overlay и отдельным олимпиадным каталогом;
- добавлен строгий recovery-протокол: исходный ответ на `original_values`, стратегия связанных параметров, 25 seed, инварианты и запрет выдумывать ответы;
- `AGENTS.md` теперь требует читать это руководство перед любой работой с шаблонами.
- устаревшие указания создавать отдельные worktree заменены правилом: один
  существующий каталог, отдельная ветка, затем проверенный merge в `main` через PR.
...
- сверены документированные контракты с `data/templates/README.md`, `problemgen/worksheet/all_tasks_site.py` и `tests/test_worksheet_site.py`;
- `python3 -m unittest tests.test_worksheet_site` — 22 теста, OK;
- `git diff --check` — OK.
- в активных инструкциях не осталось команды `git worktree add`; исторические
  упоминания сохранены только в старых записях журнала.

Заметки для следующего агента:

- не создавать новые bridge-заглушки; сначала выбрать один из трёх маршрутов из руководства;
- непроверенная архивная задача остаётся без ответа, пока не пройдёт полный recovery-протокол.
...
## 2026-07-24 — Ремонт критических дефектов массового аудита шаблонов

- Исправлены единицы и формулы поездов, воды, окрашенных/видимых кубиков, упаковки, усушки арбуза, моста, часов и турнира; также устранены связанные неоднозначности и переменные русские формы в затронутых генераторах.
- Добавлены `tests/test_audit_critical_regressions.py`, `scripts/validate_math_templates.py` и `Docs/TEMPLATE_AUDIT_REPAIR_REPORT.md`.
- Изменены генераторы модулей 11, 12, 14–20, 21–28, 30 и 31; read-only корпус `Docs/` и `docs/` не изменялся.
- Проверки: targeted 34 теста и `python -m compileall -q problemgen` — OK; `git diff --check` — OK (только Windows-предупреждения перевода строк).
- Следующему агенту: в отчёте явно отмечены четыре отдельные schema/infra-задачи (`LOG-008`, `LOG-009`, `STYLE-003`, `STYLE-004`), которые не следует маскировать текстовой правкой.
...
### Альбомная компоновка листа 15min

Дата: 2026-07-15

Что сделано:

- печать и предпросмотр переключены на A4 landscape;
- удалена верхняя черта шапки над полями ученика;
- у каждой задачи оставлена одна рабочая линия: нижняя граница блока задачи
  убрана;
- справа в шапке выделена фиксированная зона для знака «Поступление в 239» и
  QR-кода, отделённая от полей свободным пространством.
...
- ручная проверка в локальном браузере: создан вариант из 5 задач; у каждой
  задачи ровно одна рабочая линия, верхняя граница шапки отсутствует, а между
  полями и правой зоной знака/QR сохраняется свободное пространство.


### Печать пяти задач и QR Telegram-канала

Дата: 2026-07-15

Что сделано:

- печатная сетка уплотнена и зафиксирована у начала листа: пять задач больше
  не растягиваются на две страницы;
- после каждой задачи оставлена более высокая рабочая строка на всю ширину
  области заданий;
- `assets/qr.png` заново сгенерирован для `https://t.me/postuplenie239` с
  четырьмя белыми модулями тихой зоны; QR не обрезается при масштабировании.
...
- `cv2.QRCodeDetector` декодирует `assets/qr.png` в
  `https://t.me/postuplenie239`;
- ручная проверка в локальном браузере: вариант из 5 задач содержит 5
  увеличенных рабочих строк, лист остаётся альбомным, QR загружается в полном
  размере 444 × 444.


### Восстановление формул архивных задач: первый проверенный батч

Дата: 2026-07-15

Задача:

- начать разбор очищенного архива и явно описывать, как из числовых параметров
  получается ответ, не меняя исходный корпус.

Что сделано:

- добавлен `data/templates/all_tasks_answer_recovery.json` — отдельный
  накладной каталог для проверенных формул, привязанных к `template_id`;
- восстановлены 5 однозначных арифметических выражений из архива: для каждой
  записи сохранены `answer_formula`, тип ответа и контрольный ответ на
  исходных числах;
- `problemgen/worksheet/all_tasks_site.py` теперь накладывает только эти
  явные формулы, проверяет их на исходных значениях и отделяет их от ещё
  неразобранных записей;
- в ручном выборе сайта появился модуль «Архив: восстановленные формулы» с
  настоящими вычисляемыми ответами; 1083 остальных текста продолжают честно
  показываться без ответа и не участвуют в быстром варианте.
...
- `data/templates/all_tasks_answer_recovery.json` — формулы и контрольные
  ответы первого проверенного батча; его потребляет
  `problemgen/worksheet/all_tasks_site.py`.

Что нужно знать следующему агенту:

- добавлять в recovery-файл только задачи с однозначно читаемой математической
  структурой; OCR-фрагменты, неоднозначные уравнения и задачи с дополнительной
  логикой нельзя помечать вычисляемыми без отдельного валидатора;
- каждый новый recovery обязан иметь `source_answer`, который совпадает с
  результатом формулы на `original_values`.

Проверки:

- `python3 -m py_compile problemgen/worksheet/all_tasks_site.py problemgen/web/worksheet_site.py`;
- `node --check frontend/worksheet_site.js`;
- тесты сайта проверяют формулы первого батча на исходных числах и генерацию
  новых значений.


### Восстановление нелинейных архивных задач

Дата: 2026-07-15

Задача:

- продолжить восстановление формул задач, не относящихся к модулю линейных
  выражений.

Что сделано:

- в recovery-слой добавлены пять задач из трёх иных модулей: два подсчёта
  кратных в строгом интервале, подсчёт квадратов всех размеров в сетке и два
  маршрута с движением туда и обратно;
- для интервалов делимости добавлена стратегия `divisibility_interval`,
  сохраняющая порядок границ и непустой диапазон;
- для движения добавлена стратегия `roundtrip_distance`: она сначала выбирает
  целое расстояние, кратное НОК скоростей, а затем получает целое время пути;
- для сетки добавлена стратегия `square_grid_count`, сохраняющая равные
  стороны квадрата при генерации новой задачи;
- формулы ответов проверяются как на исходных, так и на новых числах.
...
- нелинейные формулы с взаимосвязанными параметрами должны сопровождаться
  именованной стратегией генерации; независимая случайная подстановка чисел
  для таких задач запрещена.

Проверки:

- tests сайта сверяют контрольные ответы и 25 новых вариантов для каждого
  восстановленного шаблона.


### Продолжение восстановления: делимость и системы

Дата: 2026-07-15

Что сделано:

- добавлена задача на подсчёт чётных чисел, кратных заданному числу, на
  замкнутом интервале;
- добавлена задача про гномов и пони с формулой решения системы по числу ног
  и голов;
- добавлена задача про угощение мальчиков и девочек: система из сумм конфет и
  котлет восстанавливает числа детей, затем считается число козинаков;
- стратегии `even_divisibility_interval`, `gnomes_and_ponies` и
  `birthday_food_system` создают согласованные параметры для новых вариантов.
...
- контрольные ответы на исходных числах и 25 вариантов с новыми значениями на
  каждый шаблон покрываются тестом сайта.


### Архив: движение и время, воркер D

Дата: 2026-07-15

Что сделано:

- в recovery-слой добавлены 10 архивных шаблонов из четырёх безопасных
  семейств: длительность олимпиады при сдвиге часовых поясов, встреча башенных
  часов с обратным ходом, погоня Гулливера за лилипутом и поездка Олега с
  возвратом через дом;
- для этих семейств добавлены стратегии `timezone_olympiad_duration`,
  `backward_tower_clock`, `gulliver_chase_steps` и `oleg_away_time`, чтобы
  новые значения сохраняли однозначную математику и целый ответ;
- расширен тест сайта: он теперь проверяет контрольные ответы 32 recovery-
  записей и отдельно валидирует инварианты новых стратегий на случайных
  генерациях.
...
- в этой ветке уже есть 32 recovery-записи и 1056 ещё неразобранных архивных
  шаблонов;
- для задач про локальное время и даты пока не добавлялись строковые ответы
  вида `HH:MM`: текущий батч ограничен только теми временными сюжетами, где
  ответ можно безопасно сохранить как целое число.

Проверки:

- `python3 -m py_compile problemgen/worksheet/all_tasks_site.py tests/test_worksheet_site.py`
- `python3 -m unittest tests.test_worksheet_site tests.test_arithmetic_templates tests.test_equation_templates tests.test_system_equation_templates tests.test_comparison_templates`


### Восстановление комбинаторики и логики: турниры, слова, включение-исключение
...
- в `data/templates/all_tasks_answer_recovery.json` добавлены 9 новых
  recovery-записей по дискретным семействам: турнир на выбывание, пересечение
  двух множеств, число слов длины до `n`, перестановки букв с повторениями и
  сумма размещений по всем длинам;
- для турниров добавлена стратегия `elimination_tournament`, которая
  генерирует только корректное число участников от 2 и выше;
- для задач на два языка добавлена стратегия `language_overlap`, которая
  сохраняет согласованные размеры групп и пересечения при генерации новых
  вариантов;
- тест сайта теперь проверяет контрольные ответы нового батча и отдельно
  валидирует обе новые стратегии генерации.
...
- multi-question шаблоны вроде «...А если участников {number_1}?» нельзя
  восстанавливать как `integer`, если из текста не следует, какая именно часть
  считается финальным ответом;
- для комбинаторных задач без числовых слотов допустим recovery с константной
  формулой, но только если число вариантов однозначно следует из literal
  текста без OCR-двусмысленности.
...
### Оптимизация позиционного сравнения цифр

Дата: 2026-07-17

Краткое имя задачи:

- исправить производительность digits-валидации перед push.

Что сделано:

- полный перебор всех чисел заменён точным перебором только ограниченных позиций с комбинаторным множителем для свободных разрядов;
- явно поддержаны позиции от единицы слева, операции `<` и `>`, чётность последней цифры и запрет совпадающих позиций;
- добавлены малые brute-force и большой 25-значный регрессионные тесты;
- исправлены три ранее найденных форматных дефекта.
...
- время позиционного подсчёта зависит от числа ограниченных разрядов, а не от величины полного числового диапазона.

Проверки:

- четыре обязательных unit-набора — 32 теста, OK;
...
### Множества, клубы, знакомства и турниры

Дата: 2026-07-19

Краткое имя задачи:

- реализовать worksheet-модуль 23 на ветке `authoring/module-23-sets`.

Что сделано:

- созданы 8 exact-integer семейств: два поля, олимпийская система, круговой турнир, клубы, языковые множества, двудольные рукопожатия, баланс очков и турнирная сумма;
- manifest учитывает все 26 уникальных источников: 22 active, 4 исключены с точными причинами;
- модуль добавлен в JSON-каталог, selector и mixed worksheet сайта;
- добавлены unit-тесты, deterministic validator и документация.
...
- два read-only source-файла модуля 23;
- `data/templates/problem_sets/sets_clubs_acquaintances_and_tournaments/README.md`;
- `templates.json` и `source_accounting.json` этого набора;
- `problemgen/generation/sets_templates.py`;
- `tests/test_sets_templates.py`;
- `scripts/validate_sets_templates.py`;
- `Docs/sets_clubs_acquaintances_and_tournaments_templates.md`.

Что нужно знать следующему агенту:

- в активных семействах нет именных ролей; при их добавлении требуется общий слой морфологии и только approved-universe data;
- №674, №1102, №1116 и №1144 намеренно не выбираются runtime, так как не задают один scalar-answer.

Проверки:

- focused unit и affected regression: 26 тестов, OK;
- validator: 8 шаблонов × 300 seed = 2400 экземпляров, OK;
- aggregate metadata: 23 модуля, 349 verified templates, OK.

### Планиметрия: прямоугольники, квадраты и площади

Дата: 2026-07-19

Что сделано:

- создано 18 exact-integer семейств периметров, площадей, разрезов, клеточных перегородок, плотности материала, ковров, рамок и единиц площади;
- manifest учитывает 75 уникальных источников: 44 active и 31 исключение с индивидуальной причиной;
- модуль добавлен в каталог, selector и mixed worksheet сайта; добавлены тесты, validator, документация и KB-карточка.

Изменённые файлы:

- `data/templates/problem_sets/catalog.json`, `problemgen/web/worksheet_site.py`, README слоёв, `tests/test_worksheet_site.py`;
- `Docs/FILE_INDEX.md`, `Docs/VECTOR_TREE.md`, `Docs/WORK_LOG.md`.

Новые файлы:

- два read-only source-файла модуля 24;
- каталог, генератор, тесты, validator и `Docs/plane_geometry_rectangles_squares_and_areas_templates.md`.

Что нужно знать следующему агенту:

- все три именованных исходника не имеют scalar-integer ответа и исключены; active-шаблоны не выбирают персонажей;
- задачи на «пузатость» исключены, потому что результатом служит отношение сторон.

Проверки:

- focused unit и affected regression: 26 тестов, OK;
- validator: 18 шаблонов × 300 seed = 5400 экземпляров, OK;
- metadata сайта: 24 модуля, 367 verified templates, OK.

### Клетчатые фигуры, разрезания и маршруты

Дата: 2026-07-19

- добавлены пять exact-integer семейств модуля 25, manifest 31 источника (19 active, 12 исключений), selector сайта, тесты и validator;
- проверки: 23 focused теста и 1500 deterministic экземпляров, OK.

### Точки, отрезки и расположение на прямой

Дата: 2026-07-19

- добавлены 6 exact-integer семейств модуля 27, manifest 15 источников (14 active, 1 конструктивное исключение), selector сайта и validator.

### Величины, единицы, вес и масштабирование

Дата: 2026-07-19

- добавлены 4 exact-integer семейства модуля 28, manifest 12 источников (7 active, 5 исключений), selector сайта, тест и validator.

### Кубы, объём и пространственная геометрия

Дата: 2026-07-19

- добавлены 5 exact-integer семейств модуля 26, manifest 44 источника (34 active, 10 исключений), selector сайта, тесты и validator;
- проверки: 23 focused теста и 1500 deterministic экземпляров, OK.
...
- добавить локальную административную Template Studio в существующий worksheet-сайт.

Что сделано:

- создан изолированный пакет `problemgen/template_studio/`: детерминированный
  анализатор, AST-вычислитель без `eval`, bounded runtime, state machine,
  audit history и atomic active overlay;
- добавлен маршрут `/admin/template-studio`, JSON API, CSRF-токен и loopback
  access restriction без фиктивной системы авторизации;
- draft проходит `draft → validated → active → archived`, поддерживает reject и
  подтверждённое удаление draft; active нельзя hard-delete;
- активированный шаблон добавляется к внутреннему выбору target module и виден
  в metadata сайта, но production JSON модулей не переписывается;
- добавлены редактор, raw JSON для advanced editing, preview, individual
  validation checks и русская документация.

Изменённые файлы:

- `problemgen/web/worksheet_site.py`, `frontend/worksheet_site.css`, README
  web и frontend;
- `Docs/FILE_INDEX.md`, `Docs/VECTOR_TREE.md`, `Docs/WORK_LOG.md`.
...
- `data/template_studio/` с README, пустым active overlay и layout-папками;
- `problemgen/template_studio/` с README и исполняемыми слоями;
- `frontend/template_studio.js`;
- `tests/test_template_studio.py`.

Что нужно знать следующему агенту:

- Studio намеренно не обещает понять произвольную математику: partial draft
  должен быть дополнен вручную и не активируется без полного validation report;
- новые subject-specific solver strategy нужно добавлять как отдельную
  детерминированную логику и тестировать, не расширять произвольный AST;
- пользовательский `Docs/AGENT_KB_BOOTSTRAP.md` в этой задаче не изменялся.

Проверки:

- Template Studio + worksheet-site regression: 36 тестов, OK;
- локальный HTTP smoke: GET `/admin/template-studio` и POST analyze с CSRF, OK;
- ручной браузерный сценарий: sequence draft → выбор модуля → validation на
  10 seed, OK; временный draft удалён;
- после ручной проверки добавлена видимая ссылка `Template Studio` в основную
  навигацию сайта; маршрут остаётся локальным `/admin/template-studio`;
- полный `unittest discover`: 263 теста, 8 ранее известных legacy-сбоев
  статического каталога; Template Studio и worksheet-site проходят;
- `git diff --check`: OK (Windows выводит только известные LF → CRLF warnings).
...
- добавлен `/admin/template-creator`: одна задача → provider-generated JSON в
  modal, validation, previews, regenerate, повторная validation, activation и delete;
- создан provider Protocol и OpenAI-compatible environment adapter без keys в
  репозитории; при отсутствии конфигурации нет поддельного fallback;
- введены strict intermediate schema, output/depth bounds, repair loop максимум
  три попытки и отдельное creator draft-хранилище;
- `arithmetic_sequence_sum` независимо проверяется явным суммированием членов;
  неизвестная стратегия/family блокирует activation.

Новые файлы:

- `Docs/TEMPLATE_CREATOR.md`, `data/template_creator/`,
  `problemgen/template_creator/`, `frontend/template_creator.js`,
  `tests/test_template_creator.py`.

Проверки:

- Template Creator + Template Studio + worksheet site: mock provider, 44 теста, OK;
- `python -m unittest discover -v`: 271 тест, 8 известных legacy-сбоев
  static catalogue (5 fail, 3 error); Creator, Studio и worksheet-site проходят;
- `python -m compileall -q problemgen`: OK;
- `git diff --check`: OK (только известные Windows LF → CRLF warnings).

Что нужно знать следующему агенту:

- полная синхронизация Trilium не выполнена: локальный Trilium server на
  `127.0.0.1:8481` не запущен; git-зеркало `knowledge/` обновлено вручную;
- legacy-сбои не относятся к этому изменению: статический каталог ожидает 1528,
  а содержит 184 template, и в нём отсутствуют analog clock/digits entries.

### Сквозной аудит русского согласования переменных

Дата: 2026-07-20

Что сделано:

- устранены формы, зависящие от числительных, в генераторах возрастов,
  календаря, часов, комбинаторики, клетчатых фигур, множеств, денег, движения,
  планиметрии, процессов, отношений, часовых поясов и работы;
- добавлена `count_phrase_ru()` для именных групп с зависимым прилагательным:
  «1 непересекающуюся квадратную дырку», «2 непересекающиеся квадратные дырки»,
  «5 непересекающихся квадратных дырок»;
- род случайного approved-персонажа в `ratios_011_box_conservation` теперь
  выбирает форму «переложил/переложила» и «проверил/проверила»;
- возрастные задачи больше не ставят approved-имя в небезопасный косвенный
  падеж: имена даны как отдельные номинативные метки ролей;
- тексты read-only исходников из `Docs/` и `docs/` не изменялись; source
  accounting продолжает сверять каждую активную стратегию с её источником.

Изменённые файлы:

- `problemgen/russian/agreement.py`;
- тематические генераторы `problemgen/generation/`;
- `tests/test_russian_variable_agreement.py`;
- `Docs/FILE_INDEX.md`, `Docs/VECTOR_TREE.md`, README русского и generation-слоёв.

Проверки:

- 58 focused unit-тестов, включая source-accounting tests, — OK;
- детерминированный аудит 2 240 экземпляров (28 selector-модулей × 80 seed)
  не нашёл ошибок форм в проверяемых номинативных счётных группах;
- полный `python -m unittest discover -s tests -v`: 275 тестов, из них 267 OK;
  оставшиеся 8 — ранее известные legacy-сбои статического каталога (ожидание
  1528 старых шаблонов при текущих 184), не связанные с этим изменением;
- `git diff --check` — OK (только известные предупреждения Windows LF → CRLF).

### Интеграция всех тематических веток в main

Дата: 2026-07-20

Что сделано:

- `main` fast-forward интегрирован по единственной зависимой цепочке веток:
  модули 09–31, selector, правки персонажей и алфавитного порядка, Template
  Studio, Automatic Template Creator и аудит русского согласования;
- реальных Git-конфликтов не было: каждая следующая ветка уже содержала
  предыдущую, поэтому изменения сохранены без дублирования;
- устранена несовместимость устаревших тестов статичного каталога с текущей
  архитектурой: compatibility-каталог валидируется своим loader-ом, а полный
  cleaned-корпус остаётся отдельным read-only source inventory;
- технические ID статичного каталога больше не выдаются за русские названия:
  тесты проверяют display `title`, а не машинный `id`.

Изменённые файлы:

- `problemgen/source_index/cleaned_problem_templates.py`;
- `tests/test_cleaned_problem_templates.py`, `tests/test_template_generator.py`;
- `problemgen/source_index/README.md`, `Docs/FILE_INDEX.md`,
  `Docs/VECTOR_TREE.md`, `Docs/WORK_LOG.md` и зеркало `knowledge/`.

Проверки:

- targeted static-catalogue regression: 30 тестов, OK;
- полный `python -m unittest discover -s tests -v`: 276 тестов, OK;
- fixed-seed selector smoke: 28 доступных модулей, OK;
- `python -m compileall -q problemgen` и `git diff --check`: OK;
- `Docs/all_tasks_all_files.md` не изменялся.

### Ремонт active templates по аудиту «three per template»

Дата: 2026-07-24

Что сделано:

- исправлены критические стратегии: знакопеременное перемещение, точный путь
  мухи, длительность поездки через абсолютное UTC-время, чётность соседних
  цифр и подсчёт чисел без запрещённой цифры;
- исправлены единицы измерения, границы клетчатых размеров, формы числительных,
  заглавная буква в начале предложения и не сокращаемые компоненты составных
  имён;
- `Кузя` явно объявлен мужским исключением к узкой эвристике рода;
- отключены `div_005_four_digit_proof` и `div_006_permutation_proof`: исходный
  read-only текст не позволяет восстановить полное доказательство без
  выдумывания математического утверждения;
- добавлены old-seed регрессии для пяти критических кодов `MATH-001`–`MATH-005`.

Изменённые файлы:

- активные стратегии в `problemgen/generation/` и JSON-каталог делимости;
- `tests/test_three_per_template_audit_regressions.py`;
- `Docs/TEMPLATE_AUDIT_REPAIR_REPORT.md`, `Docs/FILE_INDEX.md`,
  `Docs/VECTOR_TREE.md`, `Docs/WORK_LOG.md`.

Проверки:

- 57 focused unit-тестов затронутых модулей — OK;
- `python scripts/validate_math_templates.py` — 800 независимых проверок, OK;
- `Docs/all_tasks_all_files.md` и read-only исходники `docs/` не изменялись.
