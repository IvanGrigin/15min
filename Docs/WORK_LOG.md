...
### Группа H: сетки и объёмная геометрия

Дата: 2026-07-15

Задача:

- заменить bridge-заглушки листьев H01–H09 авторскими шаблонами;
- отдельно вывести и проверить формулы перегородок для прямоугольной сетки и сетки с внутренней прямоугольной дыркой.

Что сделано:

- удалены 81 bridge-шаблон `tree_h*`; добавлены 9 тематических шаблонов `h01_*`…`h09_*` для перегородок, клетчатой буквы П, распилов, брусков, сравнения распилов, окрашенного куба, расхода краски и подписей на гранях;
- добавлены solver-helper'ы `grid_internal_partitions` и `grid_partitions_with_rectangular_hole`, доступные безопасному вычислителю формул;
- H01 использует формулу `m(n−1)+n(m−1)`; H02 — формулу полного числа перегородок минус `2ab+a+b` для внутренней дырки `a×b`;
- H02 проверен независимым перебором всех внутренних прямоугольных дырок в сетках размеров 2…8 и корпусными значениями: 3×3 с дыркой 1×1 → 8, 4×4 с дыркой 2×2 → 12, 105×205 с дыркой 15×15 → 42260;
- все 158 задач H01–H09 в ворклисте получили ссылки на runtime-шаблоны и статус `done`;
- добавлен регрессионный тест с независимыми вычислениями ответов H01–H09.
...
- `tests/test_h_grid_solid_templates.py` — независимая регрессия формул и ответов шаблонов группы H.

Проверки:

- независимые проверки ответов: по 25 seed для H04–H09 и H01–H03;
- `python3 -m unittest tests.test_template_generator` — 17 тестов, OK;
- полный набор тестов нужно запускать перед публикацией PR.

Заметки для следующего агента:

- формула H02 применима к строго внутренней прямоугольной дырке; для нескольких дырок, касания края или произвольных полиомино нужны отдельные шаблоны/solver'ы;
- `Docs/AGENT_GROUP_BRIEFS.md`, упомянутый в задании, отсутствовал в `origin/main` на момент authoring; использованы дерево H и `TEMPLATE_AUTHORING_GUIDE.md`.


### Группа I: время, календарь и часы

Дата: 2026-07-15

Задача:

- после завершения H заменить bridge-заглушки следующей свободной группы I01–I08 авторскими шаблонами.

Что сделано:

- удалены 72 bridge-шаблона `tree_i*`; добавлены 8 тематических шаблонов `i01_*`…`i08_*` для дней недели, календарных дат, часовых поясов, маршрута с пересадкой, разворота, хода часов и цифрового табло;
- безопасный вычислитель расширен helper'ами для числа/даты дня недели в месяце, форматирования времени и перебора следующего табло с шестью различными цифрами;
- все 295 задач I01–I08 в ворклисте получили runtime-шаблоны и статус `done`;
- добавлена независимая регрессия временных расчётов.
...
- `tests/test_i_time_templates.py` — независимая проверка ответов группы I.

Проверки:

- независимые вычисления ответов по 25 seed для каждого из I01–I08;
- полный набор тестов нужно запускать перед публикацией PR.

Заметки для следующего агента:

- I01–I03 используют упрощённые, но однозначные условия о дне недели с номером понедельника 0;
- `next_distinct_display_seconds` ограничен двумя сутками: для текущей стратегии с максимум восемью вхождениями этого достаточно и защищает вычислитель от неограниченного поиска.
