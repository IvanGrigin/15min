...
### Авторские шаблоны группы F: комбинаторика

Что сделано:

- заменены 99 bridge-записей `tree_f01_*` … `tree_f11_*` на 11 авторских шаблонов F01–F11;
- добавлены стратегии `comb_…` для перестановок, слов, выборов, турниров, маршрутов, ладей и гарантированного выбора;
- добавлены безопасные функции формул `factorial`, `perm_multiset`, `perm_unrank`, `weighted_paths_3x3`;
- F04 выдаёт текстовый ответ (слово в восстановленном алфавитном порядке); F10 считает расстановки различимых ладей;
- в ворклисте закрыто 160 строк корпуса (F08 не имеет строк корпуса, но его bridge-шаблоны также заменены), всего `tasks_done = 194`;
- валидатор каталога дополнен текстовыми плейсхолдерами для данных стратегии без числовых constraints.
...
- для каждого из 11 семейств F ответы независимо пересчитаны/перебраны на 30 seed.

Заметки для следующего агента:

- F04 и F10 не отложены: оба типа покрыты компактными проверяемыми моделями;
- записи корпуса внутри F частично содержат старую шумную классификацию, поэтому ворклист ссылается на авторский family-template листа, а не пытается воспроизводить OCR-текст построчно.
