...
Математические шаблоны лежат отдельно в `data/templates/problem_templates.json`. Это разделение не позволяет смешать координаты листа с формулами, числами и сюжетами задач.
